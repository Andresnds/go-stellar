	...
	for iters := 0; iters < 20; iters++ {
		...
		for nth := 0; nth < npara; nth++ {
			...
			go func(me int) {
				...
				ci := (mrand.Int() % nservers)
				myck := MakeClerk([]string{lgh[ci]})
				if (mrand.Int() % 1000) < 300 {
					myck.InsertCoins("b", float32(mrand.Int()))
				} else if (mrand.Int() % 1000) < 300 {
					value := float32(mrand.Int()%10)
...
func TestGenerateAccount(t *testing.T) {
	...
	b := dsa.Verify(&priv.PublicKey, mCache.Bytes(), r, s)
	fmt.Println(b)
}
...
// 	var cka [nservers]*Clerk
// 	for i := 0; i < nservers; i++ {
// 		cka[i] = MakeClerk([]string{ledgerPort(tag, i)})
// 	}
